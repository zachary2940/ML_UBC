...
def findMinL1(funObj, w, L1_lambda, maxEvals, *args, verbose=0):
    ...
    This function has a similar interface to findMin, EXCEPT that (a) you only pass in the the function/gradientof the differentiable part,g, 
    rather than the whole function f; and (b) you need to provide the valueλ.  Toreiterate, yourfunObjshould not contain the L1 regularization term; 
    rather it should only implementthe function value and gradient for the training error term.  The reason is that the optimizer handles 
    thenon-smooth L1 regularization term in a specialized way (beyond the scope of CPSC 34This function has a similar interface tofindMin,
    EXCEPTthat (a) you only pass in the the function/gradientof the differentiable part,g, rather than the whole functionf; and (b) you need 
    to provide the valueλ.  Toreiterate, yourfunObjshould not contain the L1 regularization term; rather it should only implementthe function value 
    and gradient for the training error term.  
    The reason is that the optimizer handles thenon-smooth L1 regularization term in a specialized way (beyond the scope of CPSC 34
    """
    # Parameters of the Optimization
    optTol = 1e-2
    gamma = 1e-4
