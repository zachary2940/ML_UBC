...
\ans{
	...
	\end{bmatrix}
	\]
	Since $W^T_3\hat{x}$ is the max of all $W^T_i\hat{x}$, class label 3 would be assigned.
}
...
\\\\
\fig{1}{../figs/code_softmax1}
\fig{1}{../figs/code_softmax2}
...
\\
\ans{\\
One-vs-all training error:0.084\\
One-vs-all validation error: 0.070\\
\\
Softmax training error: 0.000\\
Softmax validation error: 0.016\\
\\
The training and validation error are identical for the one-vs-all case. For the softmax case, scikit-learn's \texttt{LogisticRegression} has the same training error but slightly higher validation error.
}
...
\ans{Relevance is not easily defined and features are only ''relevant'' in the context of available features.}
...
\ans{Missing information could make a feature relevant (Taco Tuesday problem).}
\item What is a setting where you would use the L1-loss, and what is a setting where you would use L1-regularization?
\ans{L1-loss should be used for robustness to outlier data points, and L1-regularization used for robustness to irrelevant features.}
...
\ans{L1- and L2-regularization yield convex objectives. L2-regularization yields unique solutions. L0- and L1-regularization yield sparse solutions.}
...
\ans{Increasing $\lambda$ in L1-regularization increases the sparsity level of the solution. As $\lambda$ increases, training error increases but approximation error decreases.}
...
\ans{The performance could be improved by using bootstrap samples during feature selection.}
...
\ans{It means that there is a hyperplane dividing the points into sets.}
...
\ans{Minimising the number of classification errors has a non-convex loss function.}
\item What are ``support vectors'' and what's special about them?
\ans{Support vectors are the data points that lie closest to the decision boundary, and are special as they directly influence the location of the decision boundary.}
\item What is a disadvantage of using the perceptron algorithm to fit a linear classifier?
\ans{The perceptron algorithm only works for linearly separable data.}
\item Why we would use a multi-class SVM loss instead of using binary SVMs in a one-vs-all framework?
\ans{The output of each binary SVM in a one-vs-all framework are not on the same scale as each classifier is just trying to get the sign right, and multi-class SVM loss tries to make the output of each classifier positive or negative relative to each other.}
...
\ans{$\sigma$ determines how wide the bumps are. Decreasing $\sigma$ reduces the training error, but increases the approximation error.}
}
